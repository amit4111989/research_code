...
class SdA(object):
    ...
    def anomaly_score(self,pretraining_fns,layers,batch_index,corruption_level):
         c=[]
         for i in range(layers):
            c.append(pretraining_fns[i](batch_index,corruption=corruption_level[i]))
...
def test_SdA(nins,nouts,hidden_layer_sizes,corruption_levels,
    ...
    pretraining_fns_anomaly = sda.pretraining_functions_test(train_set_x=test_set_x,
                                                batch_size=batch_size,corruption_level=corruption_levels)
	...
	#cost = sda.anomaly_score(pretraining_fns_anomaly,layers,idx,corruption_levels)
       # print len(cost[0])
        #cost = cost[0]
        #print (cost[0][90:]),
        #print (cost[0].shape)
        #cost = numpy.mean(numpy.sum(cost[0][120:]))
	if int(test_set_y[idx])==i:
	    positive_prediction+=1

	if not int(test_set_y[idx])==0 and int(test_set_y[idx])==int(i):
	    anamoly +=1
            #print('classified beat %d'),
            #print(i)
	    #print ('N beat, cost ='),
	    #normal_cost.append(cost)
            #print (cost)

        if not int(test_set_y[idx]==int(i)):
